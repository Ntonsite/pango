...
import { Building, Loader2, PartyPopper } from 'lucide-react';
    ...
    <div className="auth-container">
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="auth-form-container" style={{ borderRadius: 'var(--radius-lg)' }}>
          ...
          <h2 style={{ marginBottom: '8px', fontSize: '1.75rem', fontWeight: 700 }}>You've been invited</h2>
          <p style={{ marginBottom: '32px', color: 'var(--color-text-muted)' }}>Set a password to activate your account and get started.</p>
          ...
          {!token && (
            <div style={{ padding: '12px', backgroundColor: 'var(--color-danger-bg)', color: 'var(--color-danger)', borderRadius: 'var(--radius-md)', marginBottom: '16px', fontSize: '0.875rem' }}>
              This invite link is missing its token. Please ask for a new invite.
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn btn-primary btn-full" disabled={loading || !token} style={{ marginTop: '16px', height: '48px', fontSize: '1rem' }}>
              {loading ? (<><Loader2 className="spin" size={18} /> Activating...</>) : 'Activate Account & Sign In'}
            </button>
          </form>
